...
class Dominio(Enum):
    METADATA = (99, lambda lista_trancode, codigo_identificacao_carga, dados_todos_dominios: MetadataProcessor(lista_trancode, codigo_identificacao_carga, dados_todos_dominios), lambda dados_dominio: MetadataOrganizer(dados_dominio), lambda data_frame: MetadataDemocratizer(data_frame))
    OPERACAO = (1, lambda lista_trancode, codigo_identificacao_carga, dados_todos_dominios: OperacaoProcessor(lista_trancode, codigo_identificacao_carga, dados_todos_dominios), lambda dados_dominio: OperacaoOrganizer(dados_dominio), lambda data_frame: OperacaoDemocratizer(data_frame))
    ORIGINACAO = (2, lambda lista_trancode, codigo_identificacao_carga, dados_todos_dominios: OriginacaoProcessor(lista_trancode, codigo_identificacao_carga, dados_todos_dominios), lambda dados_dominio: OriginacaoOrganizer(dados_dominio), lambda data_frame: OriginacaoDemocratizer(data_frame))
    PARCELA = (3, lambda lista_trancode, codigo_identificacao_carga, dados_todos_dominios: ParcelaProcessor(lista_trancode, codigo_identificacao_carga, dados_todos_dominios), lambda dados_dominio: ParcelaOrganizer(dados_dominio), lambda data_frame: ParcelaDemocratizer(data_frame))
    MOVIMENTO_FINANCEIRO = (4, lambda lista_trancode, codigo_identificacao_carga, dados_todos_dominios: MovimentoFinanceiroProcessor(lista_trancode, codigo_identificacao_carga, dados_todos_dominios), lambda dados_dominio: MovimentoFinanceiroOrganizer(dados_dominio), lambda data_frame: MovimentoFinanceiroDemocratizer(data_frame))
