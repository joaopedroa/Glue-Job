...
from app.dataprovider.workflow_adapter import WorkflowAdapter
...
class JobService:
    def __init__(self):
        self.args_glue = getResolvedOptions(sys.argv, ['JOB_NAME','WORKFLOW_NAME', 'WORKFLOW_RUN_ID'])
        self.spark_context = SparkContext()
        self.glue_context = GlueContext(self.spark_context)
        self.workflow_adapter = WorkflowAdapter(self.args_glue)
    ...
    def processar_job(self):
        ...
        origem_carga = self.workflow_adapter.recuperar_origem_carga_workflow()
        ...
        dynamic_frame = self.s3_adapter.extrair(path_s3_source, "dynamic_frame_context")
        self.job.commit()
